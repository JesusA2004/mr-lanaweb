// resources/js/composables/useVacancyManager.ts
...
export function useVacancyManager(apiUrl = '/api/vacantes/gerente') {
  // ===== form state
  ...
  // ===== validation (general)
  const { errors, validate, clear: clearErrors } = useVacancyValidation<ManagerForm>()
  ...
  const rulesBase: Rule<ManagerForm>[] = [
    ...
    { field: 'office', label: 'Nivel de paquetería Office', required: true, trim: true },
  ...
  const rulesCuestionario: Rule<ManagerForm>[] = [
  ...
  function clearFieldError(key: keyof ManagerForm | 'cv') {
    clearAlerts()
    if (errors[key as string]) delete errors[key as string]
  }

  // ===== CV state (OBLIGATORIO aquí)
  ...
  function onCvChange(file: File | null) {
    ...
    if (!file) {
      errors.cv = 'Adjunta tu CV en PDF (obligatorio).'
      return
    }
  ...
  async function submit(): Promise<{ ok: boolean; message: string }> {
    ...
    // 3) valida P2–P5 (solo si no está bloqueado)
    ...
    if (result.ok) {
      ...
      // limpia CV (opcional: mantenerlo)
      cvFile.value = null
      cvName.value = ''
      return { ok: true, message: result.message }
    }

    // pinta errores 422 en el mapa local
