...
<template>
    <div class="mx-auto w-full max-w-[100rem] px-2 sm:px-3 md:px-4 lg:px-6 2xl:px-8">
        ...
        <div class="mb-3 flex flex-wrap items-center gap-2">
            <div class="inline-flex rounded-2xl border
            border-slate-200 bg-white p-1 shadow-sm">
                <button type="button" class="rounded-xl px-3 py-2 text-xs font-extrabold
                tracking-wide transition" :class="viewMode === 'map'
                ...
                <button type="button" class="rounded-xl px-3 py-2 text-xs font-extrabold
                tracking-wide transition" :class="viewMode === 'cards'
                ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900'
                : 'text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800'"
        ...
        <!-- MAPA -->
        <div v-show="viewMode === 'map'" ref="wrapRef" class="relative w-full
        overflow-hidden rounded-2xl sm:rounded-3xl border border-slate-200
        bg-white shadow-[0_18px_50px_rgba(2,6,23,0.10)]">
            <div class="p-2 sm:p-4 md:p-6">
                <div ref="svgHostRef" class="w-full select-none [touch-action:manipulation]
                min-h-[320px] sm:min-h-[420px] md:min-h-[520px] lg:min-h-[640px] xl:min-h-[720px]
                2xl:min-h-[820px]" aria-label="Mapa de sucursales por estado" role="img"/>
            </div>
        </div>
        ...
        <div v-show="viewMode === 'cards'" class="mt-4">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
                <button v-for="b in branchesSorted" :key="b.id" type="button"
                class="group text-left rounded-2xl border border-slate-200 bg-white p-4
                shadow-[0_10px_30px_rgba(2,6,23,0.08)] transition hover:-translate-y-0.5
                hover:shadow-[0_18px_45px_rgba(2,6,23,0.12)]"
                @click="focusStateFromBranch(b)">
                    ...
                    <div class="mt-3 space-y-2">
                        <div class="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-200">
                            ...
                            <div class="min-w-0">
                                <div v-if="b.phones?.length" class="flex flex-wrap gap-x-2 gap-y-1">
                                    <a v-for="ph in b.phones" :key="ph"
                                    class="font-semibold text-slate-800 hover:underline"
                                    :href="telHref(ph)" @click.stop>
                                        {{ ph }}
                                    </a>
                                </div>
                        ...
                        <a class="block text-xs font-semibold text-emerald-700 underline-offset-4
                        hover:underline" :href="mapUrlFor(b)" target="_blank" rel="noopener noreferrer"
                        @click.stop>
                            {{ b.address }}
                        </a>
                    </div>
        ...
        <!-- PANEL TELEPORT -->
        <Teleport to="body">
            <div v-show="panel.show && isMobile"
            class="fixed inset-0 z-[9998] bg-black/25 backdrop-blur-[1px]"
            @click="closePanel"/>

            <div v-show="panel.show" class="z-[9999]"
            :class="isMobile ? 'fixed inset-x-0 bottom-0 px-3 pb-3' : 'fixed'"
            :style="isMobile ? undefined : { left: panel.x + 'px', top: panel.y + 'px' }">
                <div data-branches-panel="1" class="rounded-2xl border border-slate-200 bg-white/95
                shadow-2xl backdrop-blur" :class="isMobile ? 'w-full' : 'w-[360px]'" role="dialog"
                aria-live="polite" :aria-label="`Sucursales en ${stateLabelFromCode(activeStateCode)}`"
                @click.stop>
                    <div class="flex items-start justify-between gap-3 p-3 sm:p-4">
                        ...
                        <button type="button" class="grid h-9 w-9 place-items-center rounded-full
                        border border-slate-200 bg-white text-slate-700 shadow-sm transition
                        hover:bg-slate-50" aria-label="Cerrar panel" @click="closePanel">
                            <svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    :class="isMobile ? 'max-h-[55vh] overflow-y-auto overscroll-contain' : 'max-h-[60vh] overflow-y-auto'">
                        <div v-if="activeBranches.length" class="space-y-3">
                            <div v-for="(b, idx) in activeBranches":key="b.id"
                            class="rounded-xl border border-slate-200 bg-white/70 p-3">
                                ...
                                <div class="mt-2 space-y-2">
                                    ...
                                    <div class="flex items-start gap-2 text-xs sm:text-sm text-slate-700 dark:text-slate-200">
                                        <span class="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-emerald-600/10 text-emerald-700">
                                            <svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2">
                                                <path d="M21 10c0 6-9 13-9 13S3 16 3 10a9 9 0 0 1 18 0z"></path>
                                                <circle cx="12" cy="10" r="3"></circle>
                                            </svg>
                                        ...
                                        <a class="font-semibold text-emerald-700 underline-offset-4 hover:underline" :href="mapUrlFor(b)" target="_blank"
                                ...
                                <div v-if="idx !== activeBranches.length - 1"
                                class="mt-3 h-px bg-slate-200/70"></div>
                            </div>
                        </div>
                    ...
                    <div v-if="isMobile" class="px-3 pb-3 sm:px-4 sm:pb-4">
                        <button type="button" class="w-full rounded-xl border border-slate-200
                        bg-white px-4 py-2 text-sm font-semibold text-slate-800
