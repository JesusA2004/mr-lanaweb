...
<template>
    <div class="w-full">
        <div class="space-y-3 sm:space-y-4">
            <div v-for="item in normalized" :key="item.id"
            ...
            :class="{ 'ring-2 ring-orange-200 bg-slate-50': openItem === item.id }">
                ...
                :aria-expanded="openItem === item.id">
                    ...
                    <div class="min-w-0 flex-1">
                        <h3 class="text-sm sm:text-[0.95rem] md:text-base font-extrabold
                        text-slate-900 leading-5 sm:leading-6 whitespace-normal break-words">
                            {{ item.q }}
                        </h3>
                    ...
                    <!-- Chevron -->
                    <div class="shrink-0">
                        <div class="grid h-10 w-10 place-items-center rounded-xl border
                        border-slate-200 bg-white transition group-hover:bg-slate-50">
                            <svg class="h-5 w-5 text-slate-600 transition duration-200"
                ...
                leave-to-class="opacity-0 -translate-y-1">
                    <div v-show="openItem === item.id" class="px-4 sm:px-5 md:px-6 pb-5 sm:pb-6">
                        <div class="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 md:p-6">
                            ...
                            <div v-if="typeof item.answer === 'string'"
                            ...
                            <div v-else class="space-y-4">
                                <p v-if="item.answer.length && isIntro(item.answer[0])"
                                class="text-sm sm:text-[0.95rem] md:text-base text-slate-700 leading-relaxed">
                                {{ item.answer[0] }}
                                </p>

                                <ul class="space-y-3">
                                    <li v-for="(line, i) in (item.answer.length && isIntro(item.answer[0]) ? item.answer.slice(1) : item.answer)"
                                    :key="i" class="flex items-start gap-3">
                                        <span class="mt-1 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-orange-500/10">
                                            <svg class="h-3.5 w-3.5 text-orange-600"
