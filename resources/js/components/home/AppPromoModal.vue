<script setup lang="ts">
    ...
    onMounted(() => {
      const dismissed = localStorage.getItem('mr_app_modal_dismissed')
      if (!dismissed) open.value = true
    })
    
    function close() {
      open.value = false
      localStorage.setItem('mr_app_modal_dismissed', '1')
    }
    </script>
        ...
        <div class="relative w-full max-w-lg rounded-3xl bg-white p-6 shadow-xl">
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <h3 class="mt-3 text-xl font-bold text-slate-900">Descarga la app</h3>
              <p class="mt-2 text-sm text-slate-600">
                Acceso rápido, seguimiento y contacto directo.
              </p>
            </div>
