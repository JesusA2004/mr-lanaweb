<script setup lang="ts">
    ...
    function validate(){
      ...
      if(!form.supervision) errors.supervision='Campo obligatorio'
      if(!form.inventarios) errors.inventarios='Campo obligatorio'
      ...
      <div class="w-full bg-white">
        <div class="relative w-full">
          <div class="relative h-[210px] sm:h-[230px] md:h-[250px] lg:h-[270px] xl:h-[290px] 2xl:h-[310px]">
            <img :src="bgSrc" alt="Vacante Subgerente" class="absolute inset-0 h-full w-full object-cover object-center" loading="lazy" draggable="false" />
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
              ...
              <div class="space-y-4">
                ...
                <div>
                  <label class="block font-extrabold text-slate-900 text-[12px] sm:text-[13px] md:text-[14px]">¿Has supervisado personal?</label>
                  <div class="mt-2 flex items-center gap-4 text-[13px] sm:text-[14px]">
                    <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                      <input v-model="form.supervision" type="radio" value="si" name="supervision" class="h-4 w-4 accent-emerald-500" @change="clearAlerts" />
                      <span class="font-semibold">Sí</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                      <input v-model="form.supervision" type="radio" value="no" name="supervision" class="h-4 w-4 accent-emerald-500" @change="clearAlerts" />
                      <span class="font-semibold">No</span>
                    </label>
                  </div>
                  <p v-if="errors.supervision" class="mt-1 text-[11px] sm:text-[12px] font-semibold text-rose-600">{{ errors.supervision }}</p>
                </div>

                <div>
                  <label class="block font-extrabold text-slate-900 text-[12px] sm:text-[13px] md:text-[14px]">¿Tienes experiencia con inventarios?</label>
                  <div class="mt-2 flex items-center gap-4 text-[13px] sm:text-[14px]">
                    <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                      <input v-model="form.inventarios" type="radio" value="si" name="inventarios" class="h-4 w-4 accent-emerald-500" @change="clearAlerts" />
                      <span class="font-semibold">Sí</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                      <input v-model="form.inventarios" type="radio" value="no" name="inventarios" class="h-4 w-4 accent-emerald-500" @change="clearAlerts" />
                      <span class="font-semibold">No</span>
                    </label>
                  </div>
                  <p v-if="errors.inventarios" class="mt-1 text-[11px] sm:text-[12px] font-semibold text-rose-600">{{ errors.inventarios }}</p>
                </div>
