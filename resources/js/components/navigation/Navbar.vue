...
/**
 * ✅ Fix: cuando el Sheet (Radix) está abierto, bajamos el z-index del header
 * para que el panel quede por encima (como antes).
 *
 * No tocamos MobileNavSheet ni CSS global.
 */
const sheetOpen = ref(false)
let obs: MutationObserver | null = null

function computeSheetOpen() {
  // Radix usa data-state="open" en el content; además suele poner body[data-scroll-locked]
  // Con cualquiera de los dos detectamos.
...
<template>
  ...
  <header
    :class="[
      'sticky top-0 w-full md:absolute md:inset-x-0 md:top-0',
      sheetOpen ? 'z-[10]' : 'z-[120]'
    ]"
  >
    <!-- MOBILE BAR (NO TOCAR) -->
    <div class="md:hidden w-full bg-gradient-to-r from-[#55D86A] via-[#49D7B8] shadow-[0_10px_25px_rgba(0,0,0,0.10)]">
        ...
        <div class="flex w-full items-center justify-end gap-3 py-3 flex-nowrap min-w-0">
          ...
          <button
            ...
            class="group relative inline-flex shrink-0 items-center justify-center rounded-full
                   ...
                   transition hover:bg-black/90 hover:shadow-[0_14px_34px_rgba(0,0,0,0.26)]
                   hover:-translate-y-[1px] active:translate-y-0 active:scale-[0.98]
